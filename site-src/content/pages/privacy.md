...
ANCBuddy does not use a third-party analytics SDK, ad pixels, or cross-site tracking. The first-party site events are limited to page view, trial open, trial start, download click, checkout click, and whether the optional post-purchase source question was shown, submitted, or skipped. License purchases and license validation are handled through Lemon Squeezy. For purchase-source measurement, Lemon Squeezy can send signed order webhooks to ANCBuddy's Supabase project; ANCBuddy stores the order ID, amount, status, first-party attribution fields, and a hashed customer email for matching purchases back to prior trial signups without storing the raw purchase email in the measurement table. The voluntary one-click purchase question stores one fixed source category against the order. It does not collect a name, email, license key, or free-form response, and skipping never affects the receipt, download, or license. The unpredictable order identifier is removed from the browser address bar after the page loads. ANCBuddy stores license state locally so existing users do not need to re-enter keys on every launch.
