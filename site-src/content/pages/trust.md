...
Normal mode and audio-source control is local. Quiet, Aware, Immersion, compatible Still/Motion controls, battery, connection status, and remembered-source switching use Bluetooth communication between your Mac and the paired Bose QC Ultra device. Selecting the Bose output on this Mac uses macOS audio controls.
...
The public ANCBuddy 2.2.0 DMG is signed with Developer ID, notarized by Apple, and distributed through the ANCBuddy GitHub release asset and Lemon Squeezy download flow. The app includes Sparkle update checks so fixes can ship outside the Mac App Store.
